...
#define KERNEL_SPEED 100 //ms before interrupt from kernel timer
...
static ssize_t gamepadDriverRead (struct file *filp, char __user *buff, size_t count, loff_t *offp) {
	...
	buttonData = 0xFF; //Mark buttons as "read" ("no buttons pushed")
...
static ssize_t gamepadDriverWrite (struct file *filp, const char __user *buff, size_t count, loff_t *offp)
{
	...
	switch(input){
		case '0': //Pause game
			...
			iowrite32(0x05, (volatile void *)resGPIOEvenOdd->start + GPIO_IEN); //Turn off interrupt from unecessary buttons during a pause
			break;
		case '1': //Start game
			kernelTimerRunning = 1;
			mod_timer( &gameTimer, jiffies + msecs_to_jiffies(500));
			iowrite32(0xBF, (volatile void *)resGPIOEvenOdd->start + GPIO_IEN);
			break;
		case '2': //Start kernel timer
			printk(KERN_INFO "Starting timer to fire in %dms (%ld)\n", KERNEL_SPEED, jiffies );
			kernelTimerRunning = 1;
			mod_timer( &gameTimer, jiffies + msecs_to_jiffies(KERNEL_SPEED) );
			break;
...
irqreturn_t GPIOInterruptHandler(int irq, void* dev_id, struct pt_regs* regs)
{
	...
	iowrite32(0xff, (volatile void *)resGPIOEvenOdd->start + GPIO_IFC); //Clear interrupt flag
	
	buttonData = ioread8((volatile void *)resGPIOEvenOdd->start + GPIO_PC_DIN); //Read button values
...
irqreturn_t TIMERInterruptHandler(int irq, void* dev_id, struct pt_regs* regs)
{
	...
	iowrite32(0x01, (volatile void *)resTimer3->start + TIMER3_IFC);
...
static int gamepadDriverProbe(struct platform_device *dev)
{
	...
	iowrite32(0x33333333, (volatile void *)resGPIOEvenOdd->start + GPIO_PC_MODEL);	//Enables input with filter
	iowrite32(0xFF, (volatile void *)resGPIOEvenOdd->start + GPIO_PC_DOUT);			//Enables pull-up resistors
	...
	iowrite32(0x22222222, (volatile void *)resGPIOEvenOdd->start + GPIO_EXTIPSELL);	//Selects port C for interrupts
	iowrite32(0xFF, (volatile void *)resGPIOEvenOdd->start + GPIO_EXTIFALL);			//Enables falling edge detection
	iowrite32(0xFF, (volatile void *)resGPIOEvenOdd->start + GPIO_IFC);				//Clears external interrupt flags
	iowrite32(0xBE, (volatile void *)resGPIOEvenOdd->start + GPIO_IEN);				//Enables external interrupts
...
//	iowrite32(31700, (volatile void *)resTimer3->start + TIMER3_TOP);
//	iowrite32(0x1, (volatile void *)resTimer3->start + TIMER3_IEN);
	
	/**************/
	/* Set up DAC */
	/**************/

//	iowrite32(0x50010, (volatile void *)resDAC->start + DAC0_CTRL);
//	iowrite32(0x1, (volatile void *)resDAC->start + DAC0_CH0CTRL);
//	iowrite32(0x1, (volatile void *)resDAC->start + DAC0_CH1CTRL);
