...
void pauseGame()
{
	...
	iowrite32(0x05, resGPIOEvenOdd->start + GPIO_IEN); //Turn off interrupt from unecessary buttons during a pause
...
void continueGame()
{
	...
	iowrite32(0xBF, resGPIOEvenOdd->start + GPIO_IEN);
}
...
static ssize_t gamepadDriverWrite (struct file *filp, const char __user *buff, size_t count, loff_t *offp)
{
	...
	switch(input){
		case '0': //Pause game
			//iowrite32(0x2, resTimer3->start + TIMER3_CMD); //Stop Timer3
			pauseGame();
			break;
		case '1': //Start game
			//iowrite32(0x1, resTimer3->start + TIMER3_CMD); //Start Timer3
			continueGame();
			break;
...
irqreturn_t GPIOInterruptHandler(int irq, void* dev_id, struct pt_regs* regs)
{
	...
	iowrite32(0xff, resGPIOEvenOdd->start + GPIO_IFC); //Clear interrupt flag
	
	buttonData = ioread8(resGPIOEvenOdd->start + GPIO_PC_DIN); //Read button values
...
irqreturn_t TIMERInterruptHandler(int irq, void* dev_id, struct pt_regs* regs)
{
	...
	iowrite32(0x01, resTimer3->start + TIMER3_IFC);
...
static int gamepadDriverProbe(struct platform_device *dev)
{
	...
	iowrite32(0x33333333, resGPIOEvenOdd->start + GPIO_PC_MODEL);	//Enables input with filter
	iowrite32(0xFF, resGPIOEvenOdd->start + GPIO_PC_DOUT);			//Enables pull-up resistors

	uint32_t buttonData = 0xFF;
	...
	iowrite32(0x22222222, resGPIOEvenOdd->start + GPIO_EXTIPSELL);	//Selects port C for interrupts
	iowrite32(0xFF, resGPIOEvenOdd->start + GPIO_EXTIFALL);			//Enables falling edge detection
	iowrite32(0xFF, resGPIOEvenOdd->start + GPIO_IFC);				//Clears external interrupt flags
